...
export default function Wissenswelten() {
  ...
  return <main className="worlds">
    ...
    <section className="worldHero studyRoomHero" id="top"><div className="roomSet" aria-hidden="true"><div className="roomWindow"><i/></div><div className="roomShelves"><span/><span/><span/><span/><span/><span/><span/><span/></div><div className="deskLamp"><i/><b/></div><div className="deskEdge"/></div><div className="roomTitle"><p className="worldEyebrow">Goethes digitales Studierzimmer</p><h1>Fausts<br/><em>Wissenswelten</em></h1><p className="roomQuote">Faust hat Probleme, die du kennst: zu viele Informationen, schwierige Entscheidungen und Technik mit Nebenwirkungen.</p></div><div className="worldIntro deskPaper"><span className="paperClip" aria-hidden="true">F·I</span><a className="backToReading" href="../"><span>←</span><small>Zurück zu Film & Text</small><strong>Am Anfang war der Text</strong></a><p>Hier liest du nicht nur über Faust. Du testest seine Fragen an deinem Alltag – kurz, digital und mit direktem Feedback.</p><div className="socialLegend"><span>● ALLEIN</span><span>● ZU ZWEIT</span><span>● ZU DRITT</span></div><p className="privacy">Alles bleibt lokal. Teile nur, was für dich okay ist.</p></div></section>
    <section className="library" aria-label="Module"><div className="libraryIntro"><p className="worldEyebrow">Sechs Themen · sechs digitale Experimente</p><h2>Was willst du testen?</h2><p>Such dir ein Thema aus. Jedes startet mit einer kurzen Interaktion aus deiner Lebenswelt und führt danach zurück zu Faust.</p></div><div className="moduleMap">{modules.map((m,i)=><button key={m.id} className={i===active?'active':''} onClick={()=>{setActive(i); setFilter('Alle'); document.getElementById('module')?.scrollIntoView({behavior:'smooth'});}}><span>{m.number}</span><strong>{m.title}</strong><small>{m.question}</small><b>{i===active?'Gerade geöffnet':'Experiment starten'} →</b></button>)}</div></section>
    <section className="moduleArea" id="module">
      <aside className="moduleAside"><p className="worldEyebrow">Thema {activeModule.number}</p><h2>{activeModule.title}</h2><p>{activeModule.intro}</p><dl><dt>Faust-Stellen dazu</dt><dd>{activeModule.texts}</dd><dt>Schnelleinstieg</dt><dd>5–10 Minuten fürs digitale Experiment</dd></dl><label className="teacher"><input type="checkbox" checked={teacher} onChange={e=>setTeacher(e.target.checked)}/> Hinweise für Lehrpersonen</label>{teacher&&<div className="teacherNote"><strong>Didaktischer Fokus</strong> Persönliche Beispiele bleiben freiwillig teilbar. Gruppenprodukte sind erst abgeschlossen, wenn jede Person sichtbar beigetragen hat.</div>}</aside>
      <div className="moduleContent">
        ...
        <div className="workbenchStrip"><div><span>01</span><p><strong>Selbst testen</strong>Kurze digitale Beobachtung aus deinem Alltag</p></div><div><span>02</span><p><strong>Faust-Check</strong>Dein Ergebnis am Text überprüfen</p></div><div><span>03</span><p><strong>Perspektive wechseln</strong>Mit anderen vergleichen statt Lösung erraten</p></div></div>
        <div className="taskHeading"><div><p className="worldEyebrow">Wenn du tiefer einsteigen willst</p><h2>Challenges allein, zu zweit oder zu dritt</h2></div><div className="taskFilters">{(['Alle','Selbst','Tandem','Trio'] as const).map(f=><button className={filter===f?'active':''} onClick={()=>setFilter(f)} key={f}>{f==='Selbst'?'Allein':f==='Tandem'?'Zu zweit':f==='Trio'?'Zu dritt':'Alle'}</button>)}</div></div>
