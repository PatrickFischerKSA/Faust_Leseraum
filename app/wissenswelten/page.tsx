...
export default function Wissenswelten() {
  ...
  const [textScales, setTextScales] = useState<Record<number, number>>({});
  ...
  return <main className="worlds">
    ...
    {dialog?.kind==='fact'&&<div className="dialogBackdrop" role="presentation" onMouseDown={()=>setDialog(null)}><section className="learningDialog factDialog" role="dialog" aria-modal="true" aria-labelledby="fact-dialog-title" onMouseDown={event=>event.stopPropagation()}>
      ...
      <div className="dialogBody"><div className="dialogStatement"><span>Ausgangspunkt {String(dialog.index+1).padStart(2,'0')}</span><p>{facts[dialog.index]}</p></div>{activeModule.id==='werkstatt'?<div className="faustTextBench"><header><span>Textversuch · Nacht</span><h3>Vier Sätze, vier verschiedene Reichweiten</h3><p>Ziehe jeden Regler dorthin, wo du den Satz einordnest. Links steht, was am Wortlaut und im Szenenverlauf prüfbar ist. Rechts steht ein Urteil, das weit über den Befund hinausgreift.</p></header><div className="textScaleList">{[
        ['„Habe nun, ach! Philosophie, Juristerei und Medizin … durchaus studiert“','Studieninventar'],
        ['„Da steh ich nun, ich armer Tor!“','Selbsturteil'],
        ['„Und bin so klug als wie zuvor“','Bilanz ohne sichtbaren Massstab'],
        ['„Dass wir nichts wissen können!“','Behauptung über alle Erkenntnis']
      ].map(([quote,label],index)=><label key={quote}><span><b>{index+1}</b><em>{label}</em></span><blockquote>{quote}</blockquote><input aria-label={`Reichweite von ${quote}`} type="range" min="0" max="100" value={textScales[index]??50} onChange={event=>setTextScales(current=>({...current,[index]:Number(event.target.value)}))}/><small><i>prüfbarer Befund</i><strong>{textScales[index]===undefined?'noch unentschieden':textScales[index]<34?'nah am Befund':textScales[index]<67?'deutliches Urteil':'sehr weitreichender Anspruch'}</strong><i>Totalurteil</i></small></label>)}</div><div className="counterVoices"><article><span>Wagner hält dagegen</span><p>„Zwar weiss ich viel, doch möcht’ ich alles wissen.“ Er akzeptiert vorläufiges Wissen und arbeitet weiter.</p><button onClick={()=>{const key=`fact-${activeModule.id}-${depth}-${dialog.index}`;change(key,{note:`${saved[key]?.note||''}${saved[key]?.note?' ':''}Wagner könnte einwenden: Begrenztes Wissen ist nicht Nichtwissen; sein Satz „Zwar weiss ich viel“ hält Fortschritt für möglich.`})}}>Als Gegenstimme aufnehmen</button></article><article><span>Faust verrät seine Messlatte</span><p>Er will erkennen, „was die Welt im Innersten zusammenhält“. Genügt ihm Wissen erst, wenn es das Ganze erklärt?</p><button onClick={()=>{const key=`fact-${activeModule.id}-${depth}-${dialog.index}`;change(key,{note:`${saved[key]?.note||''}${saved[key]?.note?' ':''}Fausts Satz „was die Welt im Innersten zusammenhält“ zeigt: Er misst Wissen am Anspruch auf Totalerkenntnis.`})}}>Textspur aufnehmen</button></article></div><label className="dialogWriting compact"><span>Dein Urteil – direkt am Wortlaut</span><textarea autoFocus value={saved[`fact-${activeModule.id}-${depth}-${dialog.index}`]?.note||''} onChange={e=>change(`fact-${activeModule.id}-${depth}-${dialog.index}`,{note:e.target.value})} placeholder="Faust kann begründet sagen … Zu weit geht er mit …, denn im Wortlaut … Eine haltbare Folgerung wäre …"/></label><TextFeedback prompt={`${facts[dialog.index]} Beziehe dich auf Fausts Wortlaut und unterscheide Befund, Wertung und Verallgemeinerung.`} answer={saved[`fact-${activeModule.id}-${depth}-${dialog.index}`]?.note||''}/></div>:<><div className="thinkingSteps"><article><span>1</span><div><strong>Beobachten</strong><p>Formuliere den Gedanken in einem eigenen Satz. Welche zwei Schlüsselbegriffe dürfen nicht fehlen?</p></div></article><article><span>2</span><div><strong>Prüfen</strong><p>Woran im Primärtext oder in der Edition lässt sich diese Aussage überprüfen? Notiere einen konkreten Anhaltspunkt.</p></div></article><article><span>3</span><div><strong>Weiterdenken</strong><p>Welche Grenze, Gegenposition oder offene Frage entsteht daraus?</p></div></article></div><label className="dialogWriting"><span>Deine Forschungsnotiz</span><textarea autoFocus value={saved[`fact-${activeModule.id}-${depth}-${dialog.index}`]?.note||''} onChange={e=>change(`fact-${activeModule.id}-${depth}-${dialog.index}`,{note:e.target.value})} placeholder="Eigene Formulierung – Textbezug – offene Frage …"/></label><TextFeedback prompt={facts[dialog.index]} answer={saved[`fact-${activeModule.id}-${depth}-${dialog.index}`]?.note||''}/></>}</div>
