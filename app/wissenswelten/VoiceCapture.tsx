...
const checks=(value:string,kind:'study'|'reflection')=>[
 {label:'konkret',ok:countWords(value)>=(kind==='reflection'?12:15),hint:'Nenne eine konkrete Situation oder Tätigkeit.'},
 {label:kind==='reflection'?'Erfahrung':'belegt',ok:/arbeit|journal|text|stelle|ausschnitt|beleg|beispiel|damals|als ich|phase|zeigt/i.test(value),hint:'Füge einen Beleg aus Arbeit, Journal oder eigener Erfahrung hinzu.'},
 ...
 kind==='reflection'?{label:'persönlich',ok:/ich|mich|mir|mein|meine/i.test(value),hint:'Formuliere, was du selbst bemerkt, empfunden oder gelernt hast.'}:{label:'Studienbezug',ok:/studium|studiengang|fach|modul|hochschule|universität|universitaet|beruf|disziplin|studienwahl/i.test(value),hint:'Sage ausdrücklich, was daraus für deine Studienwahl folgt.'},
];

export default function VoiceCapture({value,onChange,label,prompt,seconds=45,kind='study'}:{value:string;onChange:(value:string)=>void;label:string;prompt:string;seconds?:number;kind?:'study'|'reflection'}){
