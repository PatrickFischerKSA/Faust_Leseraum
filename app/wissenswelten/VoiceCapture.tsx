...
const checks=(value:string,kind:'study'|'reflection'|'text')=>[
 {label:'konkret',ok:countWords(value)>=(kind==='reflection'?12:15),hint:'Nenne eine konkrete Situation oder Tätigkeit.'},
 {label:kind==='reflection'?'Erfahrung':'am Material',ok:/arbeit|journal|text|stelle|ausschnitt|beleg|beispiel|aufnahme|foto|film|ton|wort|satz|szene|versuch|beobacht|zeigt|sieht|hört/i.test(value),hint:kind==='text'?'Nenne eine sicht- oder hörbare Beobachtung oder ein kurzes Textsignal.':'Füge einen Beleg aus Arbeit, Journal oder eigener Erfahrung hinzu.'},
 ...
 kind==='reflection'?{label:'persönlich',ok:/ich|mich|mir|mein|meine/i.test(value),hint:'Formuliere, was du selbst bemerkt, empfunden oder gelernt hast.'}:kind==='text'?{label:'mit Grenze',ok:/grenze|offen|unklar|unsicher|gegen|anders|nicht|nur|trotz|einerseits|andererseits/i.test(value),hint:'Sage auch, was deine Deutung nicht beweist oder was dagegen spricht.'}:{label:'Studienbezug',ok:/studium|studiengang|fach|modul|hochschule|universität|universitaet|beruf|disziplin|studienwahl/i.test(value),hint:'Sage ausdrücklich, was daraus für deine Studienwahl folgt.'},
];

export default function VoiceCapture({value,onChange,label,prompt,seconds=45,kind='study'}:{value:string;onChange:(value:string)=>void;label:string;prompt:string;seconds?:number;kind?:'study'|'reflection'|'text'}){
